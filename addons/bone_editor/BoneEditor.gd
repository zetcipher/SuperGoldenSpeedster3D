...
func _generate_bone_handles( ):
	...
	for bone_id in range( self.skeleton.get_bone_count( ) ):
		...
		if parent_bone_id == -1:
			...
			printt( bone_id, parent_bone_id, bone_name )
		...
		self.bone_handle_nodes[bone_id] = bone_handle_node
